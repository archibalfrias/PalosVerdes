...
Begin VB.Form frmOperationProShopItemsSizes 
   BackColor       =   &H00C6B8A4&
   ...
   Begin RPVGCC.b8Container picAddEdit 
      ...
      Begin RPVGCC.b8TitleBar b8TitleBar5 
         ...
         font            =   "frmOperationProShopItemsSizes.frx":0000
         fontbold        =   -1  'True
         fontname        =   "Tahoma"
         fontsize        =   8.25
         autofunction    =   0   'False
         icon            =   "frmOperationProShopItemsSizes.frx":0028
      End
   ...
   Begin VB.PictureBox picMain 
      ...
      Begin VB.CommandButton cmdClose 
         ...
         Height          =   495
         Left            =   6240
         MouseIcon       =   "frmOperationProShopItemsSizes.frx":05C4
      ...
      Begin VB.CommandButton cmdDelete 
         ...
         Height          =   495
         Left            =   6240
         MouseIcon       =   "frmOperationProShopItemsSizes.frx":08CE
         MousePointer    =   99  'Custom
         TabIndex        =   9
         Top             =   960
         Width           =   855
      ...
      Begin VB.CommandButton cmdEdit 
         ...
         Height          =   495
         Left            =   6240
         MouseIcon       =   "frmOperationProShopItemsSizes.frx":0BD8
         MousePointer    =   99  'Custom
         TabIndex        =   10
         Top             =   480
         Width           =   855
      ...
      Begin VB.CommandButton cmdAdd 
         ...
         Height          =   495
         Left            =   6240
         MouseIcon       =   "frmOperationProShopItemsSizes.frx":0EE2
         MousePointer    =   99  'Custom
         TabIndex        =   11
         Top             =   0
         Width           =   855
   ...
   End
End
Attribute VB_Name = "frmOperationProShopItemsSizes"
...
With lstRecords.ListItems
    .Clear
    s = "SELECT tbl_Operation_ProShop_Items_Sizes.* " & _
        " FROM tbl_Operation_ProShop_Items_Sizes " & _
        " WHERE (Visible = 1) " & _
        " ORDER BY ProItemsSizes "
    If rs.State = adStateOpen Then rs.Close
    rs.Open s, ConnOmega
    While Not rs.EOF
        Set x = .Add()
        x.Text = rs!PK
        x.SubItems(1) = rs!ProItemsSizes
        rs.MoveNext
    Wend
    rs.Close
End With
...
If AccessRights("Pro Shop Items (Sizes)", "Add") = False Then
...
If picAddEdit.Visible = True Then Exit Sub
If AccessRights("Pro Shop Items (Sizes)", "Delete") = False Then
    MsgBox "INSUFICIENT RIGHTS TO PERFORM THIS OPERATION.       " & vbCrLf & _
           "ACCESS DENIED!                                      ", vbCritical, "Alert"
    Exit Sub
End If
With lstRecords.ListItems
    ...
    On Error GoTo PG:
    ConnOmega.Execute "DELETE FROM tbl_Operation_ProShop_Items_Sizes WHERE (PK = " & .Item(iRow).Text & ")"
    .Remove iRow
...
If picAddEdit.Visible = True Then Exit Sub
If AccessRights("Pro Shop Items (Sizes)", "Edit") = False Then
    MsgBox "INSUFICIENT RIGHTS TO PERFORM THIS OPERATION.       " & vbCrLf & _
           "ACCESS DENIED!                                      ", vbCritical, "Alert"
    Exit Sub
End If
...
iFocus = 1
u = "SELECT tbl_Operation_ProShop_Items_Sizes.* " & _
    " FROM tbl_Operation_ProShop_Items_Sizes " & _
...
If KeyCode = vbKeyReturn Then
    On Error GoTo PG:
    If TRANSACTIONTYPE = is_ADDING Then
        ConnOmega.Execute "INSERT INTO tbl_Operation_ProShop_Items_Sizes " & _
                         " (ProItemsSizes, LastModified) " & _
                         " VALUES ('" & FORMATSQL(Trim(txtDestination.Text)) & "', " & _
                         " '" & CStr(Now) & " - " & gbl_CompleteName & "')"
        t = "SELECT PK " & _
            " FROM tbl_Operation_ProShop_Items_Sizes " & _
            " WHERE (ProItemsSizes = '" & FORMATSQL(Trim(txtDestination.Text)) & "')"
    ...
    End If
    If TRANSACTIONTYPE = is_EDITTING Then
        ConnOmega.Execute "UPDATE tbl_Operation_ProShop_Items_Sizes " & _
                         " SET ProItemsSizes = '" & FORMATSQL(Trim(txtDestination.Text)) & "', " & _
