...
Begin VB.Form frmFnBLocations 
   ...
   Begin VB.PictureBox picMain 
      ...
      Begin VB.CommandButton cmdClose 
         ...
         Height          =   495
         Left            =   6240
         MouseIcon       =   "frmFnBLocations.frx":059A
      ...
      Begin VB.CommandButton cmdDelete 
         ...
         Height          =   495
         Left            =   6240
         MouseIcon       =   "frmFnBLocations.frx":08A4
         MousePointer    =   99  'Custom
         TabIndex        =   3
         Top             =   960
         Width           =   855
      ...
      Begin VB.CommandButton cmdEdit 
         ...
         Height          =   495
         Left            =   6240
         MouseIcon       =   "frmFnBLocations.frx":0BAE
         MousePointer    =   99  'Custom
         TabIndex        =   4
         Top             =   480
         Width           =   855
      ...
      Begin VB.CommandButton cmdAdd 
         ...
         Height          =   495
         Left            =   6240
         MouseIcon       =   "frmFnBLocations.frx":0EB8
         MousePointer    =   99  'Custom
         TabIndex        =   5
         Top             =   0
         Width           =   855
   ...
   End
End
Attribute VB_Name = "frmFnBLocations"
...
With lstRecords.ListItems
    .Clear
    s = "SELECT tbl_FnB_Location.* " & _
        " FROM tbl_FnB_Location " & _
        " WHERE (Visible = 1) " & _
        " ORDER BY FnBLocation "
    If rs.State = adStateOpen Then rs.Close
    rs.Open s, ConnOmega
    While Not rs.EOF
        Set x = .Add()
        x.Text = rs!PK
        x.SubItems(1) = rs!FnBLocation
        rs.MoveNext
    Wend
    rs.Close
End With
...
If AccessRights("FnB Location", "Add") = False Then
...
If picAddEdit.Visible = True Then Exit Sub
If AccessRights("FnB Location", "Delete") = False Then
    MsgBox "INSUFICIENT RIGHTS TO PERFORM THIS OPERATION.       " & vbCrLf & _
           "ACCESS DENIED!                                      ", vbCritical, "Alert"
    Exit Sub
End If
With lstRecords.ListItems
    ...
    On Error GoTo PG:
    ConnOmega.Execute "DELETE FROM tbl_FnB_Location WHERE (PK = " & .Item(iRow).Text & ")"
    .Remove iRow
...
If picAddEdit.Visible = True Then Exit Sub
If AccessRights("FnB Location", "Edit") = False Then
    MsgBox "INSUFICIENT RIGHTS TO PERFORM THIS OPERATION.       " & vbCrLf & _
           "ACCESS DENIED!                                      ", vbCritical, "Alert"
    Exit Sub
End If
...
If KeyCode = vbKeyReturn Then
    On Error GoTo PG:
    If TRANSACTIONTYPE = is_ADDING Then
        ConnOmega.Execute "INSERT INTO tbl_FnB_Location " & _
                         " (FnBLocation, LastModified) " & _
                         " VALUES ('" & FORMATSQL(Trim(txtDestination.Text)) & "', " & _
                         " '" & CStr(Now) & " - " & gbl_CompleteName & "')"
        t = "SELECT PK " & _
            " FROM tbl_FnB_Location " & _
            " WHERE (FnBLocation = '" & FORMATSQL(Trim(txtDestination.Text)) & "')"
    ...
    End If
    If TRANSACTIONTYPE = is_EDITTING Then
        ConnOmega.Execute "UPDATE tbl_FnB_Location " & _
                         " SET FnBLocation = '" & FORMATSQL(Trim(txtDestination.Text)) & "', " & _
